...
class GuidoSettings {
	...
	private fEngine : GuidoCompiler;

	constructor ( engine: GuidoCompiler ) {
		this.fEngine = engine;
		this.initialize();
	}

	refresh() 			{ this.fEngine.refresh(); }
	get settings () : GuidoLayoutSettings 	{ return this.fEngine.settings; }
	...
	moveLinked (from: string, div: number, link: string) : number {
		let val = <number>$(from).val(); 
		$(link).val ( val ); 
	...
	get color () {
		var val =  <string>$("#score-color").val();
	...
	range (start: string, end: string) : Array<number> { 
	...
	list (head: string, tail: string) : Array<number> { 
	...
	get proll() { return this.fEngine.proll(); }
	get spr() 	{ return this.fEngine.spr(); }

	pageControl(state: boolean) : void {
	...
	initialize() {
		...
		$("#check-lyrics").click		( (event) => { this.settings.checkLyricsCollisions = $("#force-slider").is(":checked") ? 1 : 0; this.refresh(); });
